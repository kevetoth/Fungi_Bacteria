...
## Preliminary results of [Cooperation between arbuscular mycorrhizal fungi and plant growth-promoting bacteria and their effects on plant growth and soil quality](https://peerj.com/articles/13080/)
...
```{r, error=FALSE, echo=FALSE, fig.width=12, fig.height=12}
...
# <u>LDA Analyisis including all variables <u>
```{r, echo=FALSE, error=FALSE}
# Load necessary libraries
library(MASS)
library(ggplot2)
...
```{r, error=FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=24 }
...
library(ggplot2)
p1 <- ggplot(subset_data, aes(x = RootLength, y = Aboveground_Biomass, color = Group)) +
...
p2 <- ggplot(subset_data, aes(x = SRL, y = Aboveground_Biomass, color = Group)) +
  geom_point(alpha = 0.6, size = 2) + 
  geom_smooth(method = "lm", color = "red", se = FALSE) + 
...
p4 <- ggplot(subset_data, aes(x = RootLength, y = Plant_P, color = Group)) +
  geom_point(alpha = 0.6, size = 2) + 
  geom_smooth(method = "lm", color = "red", se = FALSE) + 
  annotate("text", x = min(subset_data$RootLength, na.rm = TRUE), 
...
# Fifth plot: Correlation between Total Biomass and Plant P
subset_data <- plant_soil[, c("Total_Biomass", "Plant_P", "Group")]
model <- lm(Plant_P ~ Total_Biomass, data = subset_data)
r_squared <- summary(model)$r.squared

p5 <- ggplot(subset_data, aes(x = Total_Biomass, y = Plant_P, color = Group)) +
  geom_point(alpha = 0.6, size = 2) + 
  geom_smooth(method = "lm", color = "red", se = FALSE) + 
...
# Seventh plot: Correlation between Root Branches and Aboveground Biomass
...
p7 <- ggplot(subset_data, aes(x = Rootbranches, y = Aboveground_Biomass, color = Group)) +
  geom_point(alpha = 0.6, size = 2) + 
  geom_smooth(method = "lm", color = "red", se = FALSE) + 
  ...
  labs(title = "Correlation between Root Branches and Aboveground Biomass",
       x = "Root Branches", y = "Aboveground Biomass") + 
  theme_minimal()
...
# Arrange the 8 plots in 2 columns and 4 rows
library(gridExtra)
...
The primary objective of this project was to analyze the potential synergy between arbuscular mycorrhizal fungi (AMF) and plant growth-promoting rhizobacteria (PGPR) on nutrient uptake, plant growth, and soil quality, as described in the chosen article "Cooperation between arbuscular mycorrhizal fungi and plant growth-promoting bacteria and their effects on plant growth and soil quality." We approached this hypothesis through a series of statistical and multivariate analyses, including ANOVA, PCA (Principal Component Analysis), LDA (Linear Discriminant Analysis), and correlation analyses.

1. ANOVA Analysis and Boxplots

We started by analyzing individual variables using ANOVA to determine where significant variations occurred among treatments (AMF, PGPR, and their combination).

The ANOVA results revealed significant differences in certain plant and soil traits when comparing treatments. Boxplots helped visualize where these variations occurred, particularly for treatments involving both AMF and PGPR, suggesting potential synergy.

Bonferroni corrections were applied to ensure statistical robustness, which confirmed that some results remained significant after controlling for multiple comparisons.

2. Multivariate Analyses (PCA and LDA)

To explore relationships across multiple variables simultaneously, we employed Principal Component Analysis (PCA):

PCA on plant parameters and soil parameters separately allowed us to identify how each variable contributed to PC1 and PC2. Scatterplots and arrow plots revealed strong groupings and variable influences.

A combined PCA analysis comparing PC1 of plant traits and PC1 of soil traits showed a meaningful relationship between soil quality and plant performance, supporting our hypothesis of a functional connection.

We further used Linear Discriminant Analysis (LDA) to assess how well the measured variables discriminated between treatments:

LDA scatterplots demonstrated clear group separations, with arrow plots highlighting the contribution strength of key variables to LD1 and LD2. These findings emphasized specific traits (e.g., root parameters, phosphorus content, and urease activity) as drivers of treatment differences.

3. Correlation Analyses

To explore interdependencies between traits, we conducted correlograms for plant characteristics and soil parameters, specific Root Length (SRL) correlations with plant and soil variables.

These analyses revealed strong correlations, particularly highlighting SRL’s role as a key trait influencing both plant growth and soil quality. Additionally, nonlinear correlations, such as between urease activity (UE) and plant phosphorus, were identified, showcasing complex relationships that might not have been captured in linear models.
