...
# Afficher le tbl_df dans un tableau formaté
```{r display_tbl_kable}
library(knitr)
kable(plant_soil, caption = "plant_soil")
```
...
knitr::include_graphics("Significant_ANOVA_Boxplots.pdf")
```
...
To be able to display LDA of all the variables 
```{r}
# Load necessary libraries
library(MASS)
library(ggplot2)
...
1 Correlogram of specific root length correlation to plant parameters and 1 different correlogram for specific root length to soil parameters
