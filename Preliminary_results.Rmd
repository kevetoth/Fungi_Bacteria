...
<<<<<<< Updated upstream
# Load the libraries
library(pdftools)  # To read and render PDF pages
library(magick)    # To manipulate images
library(knitr)     # To include images in the HTML

# Specify the path to your PDF file
pdf_file <- "/Users/CAMILLE/Desktop/Fungi_Bacteria/Significant_ANOVA_mix.pdf"

# Convert the first page to a raster image (matrix)
page1_raster <- pdf_render_page(pdf_file, page = 1, dpi = 150)

# Convert the raster matrix to a magick object
...
# Include the image in the HTML document
=======
# Load libraries for this chunk
library(pdftools)  # For rendering PDF pages
library(magick)    # For image manipulation
library(knitr)     # For including images in HTML

# Dynamically locate the PDF file in the main folder
pdf_file <- here("Significant_ANOVA_mix.pdf")

# Verify the file exists
if (!file.exists(pdf_file)) {
  stop("File not found: ", pdf_file)
}

# Render the first page of the PDF to a raster
page1_raster <- pdf_render_page(pdf_file, page = 1, dpi = 150)
...
dir.create(output_dir, showWarnings = FALSE)  # Create 'output' folder if missing
image_path <- file.path(output_dir, "page1_image.png")
image_write(page1_image, path = image_path, format = "png")
...
# Define the path to your PDF file using `here`
pdf_file <- here("PCA_Plant.pdf")
...
# Save the image to the output folder as a PNG file
output_dir <- here("output")
dir.create(output_dir, showWarnings = FALSE)  # Create 'output' folder if missing
image_path <- file.path(output_dir, "PCA_Plant_Page1.png")
image_write(page1_image, path = image_path, format = "png")
...
# PCA Analysis (for the Soil, Plant, Soil_Loadings, Plant_Loadings)

<iframe src="PCA_Plant.pdf" width="600" height="510"></iframe>

<iframe src="PCA_Plant_loadings.pdf" width="600" height="500"></iframe>

<iframe src="PCA_Soil.pdf" width="600" height="510"></iframe>

<iframe src="PCA_Soil_loadings.pdf" width="600" height="510"></iframe>
...
# Save the correlogram to a file in the output directory
output_dir <- here("output")
dir.create(output_dir, showWarnings = FALSE)  # Ensure 'output' folder exists
ggsave(here(output_dir, "Correlogram_plant_vs_soil.pdf"), plot = correlogram, width = 10, height = 8)

# Define path to the saved PDF (using `here()` for dynamic path resolution)
pdf_path <- here(output_dir, "Correlogram_plant_vs_soil.pdf")

# Convert the PDF to a large PNG for embedding
png_file <- here(output_dir, "correlogram_plant_vs_soil_large.png")

# Convert the first page of the PDF to PNG
pdf_convert(pdf_path, format = "png", pages = 1, dpi = 300, filenames = png_file)

# Display the PNG image in the HTML document
...
pdf_path <- "Correlogram_plant_vs_soil.pdf"
...
### Discussion. From your objectives
The primary objective of this project was to analyze the potential
synergy between arbuscular mycorrhizal fungi (AMF) and plant
growth-promoting rhizobacteria (PGPR) on nutrient uptake, plant
growth, and soil quality,  
as described in the chosen article "Cooperation between arbuscular
mycorrhizal fungi and plant growth-promoting bacteria and their
effects on plant growth and soil quality."
We approached this hypothesis through a series of statistical and
multivariate analyses, including ANOVA, PCA (Principal Component
Analysis), LDA (Linear Discriminant Analysis), and correlation
analyses.
...
### Conclusion
Through statistical and multivariate analyses, this project supported
the hypothesis that the combined application of AMF and PGPR enhances
plant growth and soil quality more effectively than individual
treatments: Significant variations observed in plant traits and soil
parameters confirmed that AMF and PGPR, when applied together,
exhibit synergistic effects.PCA and LDA analyses highlighted
relationships between plant performance and soil quality, identifying
key drivers such as phosphorus uptake, urease activity, and specific
root length (SRL). Correlations between variables, including
nonlinear trends, underscored the complexity of interactions in
plant-soil systems and focus attention on the need for multivariate
approaches to understand these dynamics fully.
