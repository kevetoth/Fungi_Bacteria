...
correlation <- cor(plant_soil$UE, plant_soil$Plant_P, use = "complete.obs")

# Calculate R-squared
r_squared <- correlation^2

# Create the correlation plot
correlation_plot <- ggplot(plant_soil, aes(x = UE, y = Plant_P)) +
  geom_point(color = "blue", size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +  # Add regression line
  labs(title = "Correlation Plot: UE vs Plant_P",
       subtitle = paste("Coefficient of Determination (R²):", round(r_squared, 3)),
       x = "UE",
       y = "Plant_P") +
  theme_minimal() +
...
poly_model <- lm(Plant_P ~ poly(UE, 2), data = plant_soil)

# Extract coefficients and create equation text
coefs <- coef(poly_model)
equation <- paste0("y = ", signif(coefs[1], 3), 
                   " + ", signif(coefs[2], 3), "x", 
                   " + ", signif(coefs[3], 3), "x²")

# Calculate R-squared
r_squared <- signif(summary(poly_model)$r.squared, 3)

# Combine equation and R-squared
equation_with_r2 <- paste0(equation, "\nR² = ", r_squared)

# Plot with LOESS and polynomial equation
ggplot(plant_soil, aes(x = UE, y = Plant_P)) +
  geom_point(color = "blue", size = 3, alpha = 0.7) +
  geom_smooth(method = "loess", color = "green", se = TRUE) +
  stat_smooth(method = "lm", formula = y ~ poly(x, 2), color = "purple", se = FALSE) +
  annotate("text", x = mean(plant_soil$UE), y = max(plant_soil$Plant_P, na.rm = TRUE), 
           label = equation_with_r2, color = "purple", size = 4, hjust = 0) +
  labs(title = "LOESS Curve with Polynomial Approximation",
       x = "UE",
       y = "Plant_P") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
...
In this data analysis project we evaluated the effects of Arbuscular Mycorrizhal Fungi AMF and Plant Growth Promoting Rhizobacteria PGPB on root morphology, plant growth, and soil properties. Our data analysis confirms the findings of the original research. Similar to their results, we observed that AMF treatments had the strongest impact, improving biomass, phosphorus uptake, and soil properties. The combined treatment of AMF and PGPB provided additional benefits, such as enhanced enzyme activity like Urease Activity (UE) and Alkaline Phosphatase (ALP). Specific Root Length (SRL) was identified as a key trait for nutrient uptake, although it was linked to reduced root mass. The correlation between urease activity and phosphorus highlighted complex nutrient interactions influenced by the treatments. These findings confirmed the conclusion of the original study that dual inoculation of AMF and PGPB is the most effective strategy for improving plant growth, nutrient acquisition and soil quality. Important to add is that if the goal is to only increase plant total biomass, using only AMF can be more cost-effective option as it provides similar results to the combined treatment of AMF and PGPB. Overall, this approach offers potential as a bio-fertilizer to promote sustainable forage production and soil health, particularly in regions like the Qinghai-Tibet Plateau.
.........................................
